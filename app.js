...
const pictureTasks = [
  {
    title: "Familie im Zoo",
    image: "assets/photos/photo-01.jpg",
    fallback: "Eine Familie besucht den Zoo und schaut sich Tiere an.",
    tr: "Bir aile hayvanat bahçesinde hayvanlara bakıyor.",
    expectedKeywords: ["familie", "zoo", "kinder", "tiere", "ausflug", "beobachten"],
  },
  {
    title: "Gespräch auf dem Balkon",
    image: "assets/photos/photo-02.jpg",
    fallback: "Zwei ältere Personen sitzen auf dem Balkon und unterhalten sich.",
    tr: "İki yaşlı kişi balkonda oturup konuşuyor.",
    expectedKeywords: ["balkon", "gespräch", "senioren", "nachbarn", "wohnen", "tee"],
  },
  {
    title: "Fahrradtraining für Kinder",
    image: "assets/photos/photo-03.jpg",
    fallback: "Kinder üben mit Fahrrädern auf einem Verkehrsübungsplatz.",
    tr: "Çocuklar trafik eğitim alanında bisiklet sürmeyi öğreniyor.",
    expectedKeywords: ["kinder", "fahrrad", "verkehr", "training", "helm", "sicherheit"],
  },
  {
    title: "Familienessen im Restaurant",
    image: "assets/photos/photo-04.jpg",
    fallback: "Eine Familie sitzt gemeinsam im Restaurant und isst.",
    tr: "Bir aile restoranda birlikte yemek yiyor.",
    expectedKeywords: ["familie", "restaurant", "essen", "kinder", "gemeinsam", "tisch"],
  },
  {
    title: "Arbeit in der Gastronomie",
    image: "assets/photos/photo-05.jpg",
    fallback: "Ein Mann arbeitet an einer Bar und bereitet Getränke zu.",
    tr: "Bir adam barda çalışıyor ve içecek hazırlıyor.",
    expectedKeywords: ["arbeit", "gastronomie", "barkeeper", "getränke", "beruf", "service"],
  },
];
...
*/
(() => {
  const SMART_VERSION = "20260623-fotos-20";
  ...
  const photoGroups = [
    { title: "Familie im Zoo", keywords: ["familie", "zoo", "kinder", "tiere", "ausflug", "beobachten", "freizeit", "eltern"] },
    { title: "Gespräch auf dem Balkon", keywords: ["balkon", "gespräch", "senioren", "nachbarn", "wohnen", "tee", "freizeit", "unterhalten"] },
    { title: "Fahrradtraining für Kinder", keywords: ["kinder", "fahrrad", "verkehr", "training", "sicherheit", "lernen", "helm", "schilder"] },
    { title: "Familienessen im Restaurant", keywords: ["familie", "restaurant", "essen", "kinder", "gemeinsam", "feiern", "tisch", "bestellen"] },
    { title: "Arbeit in der Gastronomie", keywords: ["arbeit", "gastronomie", "barkeeper", "getränke", "beruf", "service", "bar", "mischen"] },
    { title: "Kindergruppe unterwegs", keywords: ["kinder", "kita", "erzieherin", "ausflug", "gruppe", "unterwegs", "rucksack", "weg"] },
    { title: "Rettungswagen im Verkehr", keywords: ["rettungswagen", "verkehr", "krankenwagen", "straße", "notfall", "autos", "sirene", "hilfe"] },
    { title: "Kinder auf dem Spielplatz", keywords: ["kinder", "spielplatz", "spielen", "rutsche", "draußen", "freizeit", "klettern", "park"] },
    { title: "Spaziergang am See", keywords: ["paar", "hund", "see", "spaziergang", "natur", "freizeit", "wandern", "weg"] },
    { title: "Unterricht in der Schule", keywords: ["schule", "unterricht", "lehrerin", "kinder", "lernen", "klasse", "tafel", "erklären"] },
    { title: "Wäsche waschen", keywords: ["wäsche", "waschmaschine", "kleidung", "haushalt", "waschen", "zu hause", "arbeit", "helfen"] },
    { title: "Einsteigen in den Bus", keywords: ["bus", "haltestelle", "einsteigen", "verkehr", "fahrgäste", "öffentliche verkehrsmittel", "warten", "stadt"] },
    { title: "Einkaufen in der Stadt", keywords: ["einkaufen", "stadt", "taschen", "geschäft", "schaufenster", "kleidung", "freizeit", "kaufen"] },
    { title: "Fernsehen zu Hause", keywords: ["fernsehen", "fernbedienung", "zu hause", "medien", "freizeit", "bildschirm", "sendung", "auswählen"] },
    { title: "Joggen im Park", keywords: ["joggen", "sport", "park", "gesundheit", "laufen", "paar", "freizeit", "bewegung"] },
    { title: "Deutsch lernen zu Hause", keywords: ["lernen", "deutsch", "schreiben", "heft", "bücher", "zu hause", "üben", "prüfung"] },
    { title: "Sport zu Hause", keywords: ["sport", "yoga", "matte", "zu hause", "gesundheit", "übung", "bewegung", "training"] },
    { title: "Einkaufen im Supermarkt", keywords: ["supermarkt", "einkaufen", "obst", "gemüse", "wagen", "preise", "auswählen", "lebensmittel"] },
    { title: "Gemüse vorbereiten", keywords: ["küche", "gemüse", "kochen", "schneiden", "paprika", "salat", "essen", "vorbereiten"] },
    { title: "Geschirr spülen", keywords: ["geschirr", "spülen", "küche", "haushalt", "teller", "wasser", "aufräumen", "helfen"] },
  ];

  const smartPhotoTasks = photoGroups.map((group, index) => {
    const number = index + 1;
    return {
      id: `photo-${number}`,
      title: group.title,
  ...
  function smartSpeakQueue(items) {
    const queue = (Array.isArray(items) ? items : [items]).map((item) => String(item || "").trim()).filter(Boolean);
    ...
    const playNext = () => {
      ...
      utterance.lang = "de-DE";
      utterance.rate = 0.9;
      utterance.onend = () => window.setTimeout(playNext, 450);
      utterance.onerror = () => window.setTimeout(playNext, 450);
      window.speechSynthesis.speak(utterance);
    };
    playNext();
  }
    ...
    return `
      <section class="hero-panel">
        ...
        <div class="stat-grid">
          ...
          <div><strong>${smartPhotoTasks.length}</strong><span>Fotoaufgaben mit Aufnahme</span></div>
  ...
  function renderDashboard() {
    return `
      ...
      <section class="grid">
        ${[
          ["photos", "Fotos beschreiben", `${smartPhotoTasks.length} echte Bildaufgaben: beschreiben, Erfahrung erzählen, Heimatland vergleichen.`],
  ...
  function renderListening() {
    const blockStart = smartRuntime.listeningIndex;
    const blockSize = 4;
    const block = listeningPractice.slice(blockStart, Math.min(listeningPractice.length, blockStart + blockSize));
    const audioId = `listen-block-${blockStart}`;
    smartAudio.set(audioId, block.map((task, offset) => `Aufgabe ${blockStart + offset + 1}. ${task.audio}`));
    return `
      <section class="exam-paper">
        <div class="exam-paper-header">
          <div>
            <p class="eyebrow">Hören</p>
            <h2>Hörteil nacheinander</h2>
          </div>
          <span>Aufgaben ${blockStart + 1} bis ${blockStart + block.length} von ${listeningPractice.length}</span>
        </div>
        <div class="audio-card">
          <div>
            <strong>Hörblock</strong>
            <button class="primary" type="button" data-play-audio-sequence="${h(audioId)}">Hörproben nacheinander starten</button>
            <button class="ghost" type="button" data-stop-audio>Stoppen</button>
          </div>
          <p class="muted">Die App sagt zuerst die Aufgabennummer und spielt danach automatisch die passende Hörprobe ab. Danach kommt die nächste Aufgabe.</p>
        </div>
        ${block.map((task, offset) => {
          const taskIndex = blockStart + offset;
          return `
            <article class="exam-task">
              <h3>${taskIndex + 1}. ${h(task.title)}</h3>
              <p class="muted">Hören Sie die passende Hörprobe und beantworten Sie die Fragen.</p>
              ${task.questions.map((question, questionIndex) => renderQuestion(`listen-${taskIndex}-${questionIndex}`, question[0], question[1], question[2])).join("")}
              <div class="actions">
                <button type="button" data-check-section="listen-${taskIndex}">Aufgabe ${taskIndex + 1} prüfen</button>
              </div>
              <div id="sectionFeedback-listen-${taskIndex}"></div>
            </article>
          `;
        }).join("")}
        <div class="actions">
          <button class="ghost" type="button" data-next-listening="${block.length}">Nächste Hörgruppe</button>
        </div>
      </section>
    `;
  }
  ...
  function renderExamGroup(title, tasks, examNumber) {
    const listeningTasks = tasks.filter((task) => task.audio);
    const audioId = `exam-${examNumber}-audio-block-${tasks[0]?.number || 0}`;
    if (listeningTasks.length) {
      smartAudio.set(audioId, listeningTasks.map((task) => `Aufgabe ${task.number}. ${task.audio}`));
    }
    return `
      <section class="exam-section">
        <div class="section-ribbon">${h(title)}</div>
        ${listeningTasks.length ? `
          <div class="audio-card">
            <div>
              <strong>Hörteil</strong>
              <button class="primary" type="button" data-play-audio-sequence="${h(audioId)}">Hörteil nacheinander starten</button>
              <button class="ghost" type="button" data-stop-audio>Stoppen</button>
            </div>
            <p class="muted">Die Hörproben laufen wie in der Prüfung nacheinander: Aufgabe, Hörprobe, nächste Aufgabe.</p>
          </div>
        ` : ""}
    ...
    if (task.audio) {
      return `
        <article class="exam-task" data-exam-task="${task.number}" data-answer="${h(task.answer)}" data-kind="auto">
          ...
          <p>${h(task.title)}</p>
          <p class="muted">Diese Hörprobe wird im Hörteil automatisch an der passenden Stelle abgespielt.</p>
  ...
  function bindSmartView() {
    ...
    document.querySelectorAll("[data-play-audio-sequence]").forEach((button) => button.addEventListener("click", () => smartSpeakQueue(smartAudio.get(button.dataset.playAudioSequence) || [])));
    document.querySelectorAll("[data-stop-audio]").forEach((button) => button.addEventListener("click", () => {
    ...
    document.querySelectorAll("[data-next-listening]").forEach((button) => button.addEventListener("click", () => {
      const step = Number(button.dataset.nextListening) || 1;
      smartRuntime.listeningIndex = (smartRuntime.listeningIndex + step) % listeningPractice.length;
